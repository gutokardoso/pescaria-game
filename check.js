    ...
    function announceGiantSpawn() {
      document.querySelectorAll('.giant-spawn-alert').forEach(alert => alert.remove());
    ...
    function forceHudLayout() {
      ...
      if (mission) {
        mission.style.setProperty('left', '8px', 'important');
        mission.style.setProperty('right', 'auto', 'important');
        mission.style.setProperty('top', '6px', 'important');
        mission.style.setProperty('width', 'clamp(560px, 56vw, 650px)', 'important');
        mission.style.setProperty('min-width', '560px', 'important');
        ...
        mission.style.setProperty('background-image', "url('./assets/hud-missao-perfil-final.png')", 'important');
        mission.style.setProperty('background-size', 'contain', 'important');
        mission.style.setProperty('background-repeat', 'no-repeat', 'important');
        mission.style.setProperty('background-position', 'left top', 'important');
    ...
    function showGiantQuickNotice() {
      ...
      notice.className = 'combo-pop giant-quick-notice';
      notice.textContent = 'PEIXE GIGANTE!';
      game.appendChild(notice);
...
applyHookVisuals();
    ...
    function finalHudForceLayout() {
      const mission = document.getElementById('missionProgress');
      const label = document.getElementById('missionProgressLabel');
      const score = document.getElementById('liveScore');
      const gameEl = document.getElementById('game');

      const w = gameEl && gameEl.getBoundingClientRect ? gameEl.getBoundingClientRect().width : window.innerWidth;
      const isSmall = w <= 720;

      if (mission) {
        mission.style.setProperty('position', 'absolute', 'important');
        mission.style.setProperty('top', isSmall ? '38px' : '42px', 'important');
        mission.style.setProperty('left', isSmall ? '18px' : '32px', 'important');
        mission.style.setProperty('right', 'auto', 'important');
        mission.style.setProperty('bottom', 'auto', 'important');
        mission.style.setProperty('width', isSmall ? '230px' : '300px', 'important');
        mission.style.setProperty('max-width', isSmall ? '34%' : '30%', 'important');
        mission.style.setProperty('height', isSmall ? '30px' : '34px', 'important');
        mission.style.setProperty('transform', 'none', 'important');
        mission.style.setProperty('margin', '0', 'important');
        mission.style.setProperty('z-index', '9000', 'important');
      }

      if (label) {
        label.style.setProperty('font-size', isSmall ? '11px' : '14px', 'important');
        label.style.setProperty('line-height', isSmall ? '30px' : '34px', 'important');
      ...
      if (score) {
        ...
        score.style.setProperty('top', isSmall ? '38px' : '42px', 'important');
        score.style.setProperty('left', '50%', 'important');
        score.style.setProperty('right', 'auto', 'important');
        score.style.setProperty('bottom', 'auto', 'important');
        score.style.setProperty('transform', 'translateX(-50%)', 'important');
        score.style.setProperty('margin', '0', 'important');
        score.style.setProperty('z-index', '9001', 'important');
      }
    }

    function finalSetGameplayHudVisible(visible) {
      const depthEls = [
        document.getElementById('depthMeter'),
        document.getElementById('depthNumber'),
        ...Array.from(document.querySelectorAll('.depth-meter, .depth-number'))
      ];

      depthEls.forEach(el => {
        if (!el) return;
        el.classList.toggle('depth-ui-hidden', !visible);
      ...
      const mission = document.getElementById('missionProgress');
      if (mission) {
        mission.classList.toggle('mission-ui-hidden', !visible);
      ...
      if (visible) finalHudForceLayout();
    }

    function finalSetScreen(screenName) {
      document.body.classList.remove('screen-start', 'screen-prelevel', 'screen-playing', 'screen-result', 'screen-shop');
      document.body.classList.add(`screen-${screenName}`);
      finalSetGameplayHudVisible(screenName === 'playing');
      finalHudForceLayout();
    }

    const finalOldStartRound = typeof startRound === 'function' ? startRound : null;
    if (finalOldStartRound) {
      startRound = function(...args) {
        const result = finalOldStartRound.apply(this, args);
        finalSetScreen('playing');
        setTimeout(() => finalSetScreen('playing'), 60);
        setTimeout(finalHudForceLayout, 140);
        return result;
      };
    }

    const finalOldFinishRound = typeof finishRound === 'function' ? finishRound : null;
    if (finalOldFinishRound) {
      finishRound = async function(...args) {
        finalSetScreen('result');
        return await finalOldFinishRound.apply(this, args);
      };
    }

    const finalOldShowPreLevelScreen = typeof showPreLevelScreen === 'function' ? showPreLevelScreen : null;
    if (finalOldShowPreLevelScreen) {
      showPreLevelScreen = function(...args) {
        const result = finalOldShowPreLevelScreen.apply(this, args);
        finalSetScreen('prelevel');
        return result;
      };
    }

    const finalOldOpenShop = typeof openShop === 'function' ? openShop : null;
    if (finalOldOpenShop) {
      openShop = function(...args) {
        const result = finalOldOpenShop.apply(this, args);
        finalSetScreen('shop');
        return result;
      };
    }

    const finalOldCloseShop = typeof closeShop === 'function' ? closeShop : null;
    if (finalOldCloseShop) {
      closeShop = function(...args) {
        const result = finalOldCloseShop.apply(this, args);
        const startVisible = startOverlay && !startOverlay.classList.contains('hidden');
        finalSetScreen(startVisible ? 'start' : 'playing');
        return result;
      };
    }

    const finalOldUpdateMissionProgress = typeof updateMissionProgress === 'function' ? updateMissionProgress : null;
    if (finalOldUpdateMissionProgress) {
      updateMissionProgress = function(...args) {
        const result = finalOldUpdateMissionProgress.apply(this, args);
        if (document.body.classList.contains('screen-playing')) {
          finalSetGameplayHudVisible(true);
        }
        finalHudForceLayout();
        return result;
      };
    ...
    finalSetScreen('start');
    setTimeout(() => finalSetScreen('start'), 100);


    function finalAlignHudSameLine() {
      const gameEl = document.getElementById('game');
      const width = gameEl && gameEl.getBoundingClientRect ? gameEl.getBoundingClientRect().width : window.innerWidth;
      const isSmall = width <= 720;
      const topLine = isSmall ? '66px' : '78px';
      ...
      const coinCandidates = [
        document.getElementById('liveCoins'),
        ...Array.from(document.querySelectorAll('.coin-hud, .coins-hud'))
      ].filter(Boolean);

      if (mission) {
        mission.style.setProperty('position', 'absolute', 'important');
        mission.style.setProperty('top', topLine, 'important');
        mission.style.setProperty('left', isSmall ? '18px' : '32px', 'important');
        mission.style.setProperty('right', 'auto', 'important');
        mission.style.setProperty('bottom', 'auto', 'important');
        mission.style.setProperty('width', isSmall ? '230px' : '300px', 'important');
        mission.style.setProperty('max-width', isSmall ? '34%' : '30%', 'important');
        mission.style.setProperty('height', isSmall ? '30px' : '34px', 'important');
        mission.style.setProperty('transform', 'translateY(-50%)', 'important');
        mission.style.setProperty('margin', '0', 'important');
      }
      ...
      if (score) {
        score.style.setProperty('position', 'absolute', 'important');
        score.style.setProperty('top', topLine, 'important');
        score.style.setProperty('left', '50%', 'important');
        score.style.setProperty('right', 'auto', 'important');
        score.style.setProperty('bottom', 'auto', 'important');
        score.style.setProperty('transform', 'translate(-50%, -50%)', 'important');
        score.style.setProperty('display', 'flex', 'important');
        score.style.setProperty('align-items', 'center', 'important');
        score.style.setProperty('justify-content', 'center', 'important');
      }

      coinCandidates.forEach(coin => {
        coin.style.setProperty('position', 'absolute', 'important');
        coin.style.setProperty('top', topLine, 'important');
        coin.style.setProperty('right', isSmall ? '18px' : '34px', 'important');
        coin.style.setProperty('left', 'auto', 'important');
        coin.style.setProperty('bottom', 'auto', 'important');
        coin.style.setProperty('transform', 'translateY(-50%)', 'important');
        coin.style.setProperty('display', 'flex', 'important');
        coin.style.setProperty('align-items', 'center', 'important');
        coin.style.setProperty('justify-content', 'center', 'important');
      });
    }

    const __oldFinalHudForceLayout = typeof finalHudForceLayout === 'function' ? finalHudForceLayout : null;
    if (__oldFinalHudForceLayout) {
      finalHudForceLayout = function(...args) {
        const result = __oldFinalHudForceLayout.apply(this, args);
        finalAlignHudSameLine();
        return result;
      };
    }

    const __oldForceTopHudLayout = typeof __forceTopHudLayout === 'function' ? __forceTopHudLayout : null;
    if (__oldForceTopHudLayout) {
      __forceTopHudLayout = function(...args) {
        const result = __oldForceTopHudLayout.apply(this, args);
        finalAlignHudSameLine();
        return result;
      };
    }
    ...
    finalAlignHudSameLine();
    setTimeout(finalAlignHudSameLine, 120);


    function finalCompactMissionLabel(text) {
      ...
      if (lower.includes('dourado')) label = 'Peixes dourados';
      else if (lower.includes('especial') || lower.includes('especiais') || lower.includes('garrafa') || lower.includes('baú') || lower.includes('bau')) label = 'Itens especiais';
      else if (lower.includes('cavalo')) label = 'Cavalos-marinhos';
      else if (lower.includes('tartaruga')) label = 'Tartarugas-marinhas';
      else if (lower.includes('tubar')) label = 'Tubarões';
      else if (lower.includes('gigante')) label = 'Peixe gigante';

      return `${label}: ${current}/${target}`;
    }

    function finalAlignHudPerfect() {
      const gameEl = document.getElementById('game');
      const width = gameEl && gameEl.getBoundingClientRect ? gameEl.getBoundingClientRect().width : window.innerWidth;
      const isSmall = width <= 720;
      const topLine = isSmall ? '52px' : '58px';
      ...
      if (label && label.textContent) {
        label.textContent = finalCompactMissionLabel(label.textContent);
      }

      if (mission) {
        mission.style.setProperty('position', 'absolute', 'important');
        mission.style.setProperty('top', topLine, 'important');
        mission.style.setProperty('left', isSmall ? '16px' : '32px', 'important');
        mission.style.setProperty('width', isSmall ? '230px' : '310px', 'important');
        mission.style.setProperty('height', isSmall ? '30px' : '34px', 'important');
        mission.style.setProperty('transform', 'translateY(-50%)', 'important');
        mission.style.setProperty('margin', '0', 'important');
      }
      ...
      if (label) {
        label.style.setProperty('line-height', isSmall ? '30px' : '34px', 'important');
        label.style.setProperty('font-size', isSmall ? '12px' : '15px', 'important');
      }
      ...
      coins.forEach(coin => {
        ...
        coin.style.setProperty('align-items', 'center', 'important');
        coin.style.setProperty('justify-content', 'center', 'important');
        coin.style.setProperty('line-height', '1', 'important');
      });
    }

    const oldFinalAlignHudSameLine = typeof finalAlignHudSameLine === 'function' ? finalAlignHudSameLine : null;
    if (oldFinalAlignHudSameLine) {
      finalAlignHudSameLine = function(...args) {
        const result = oldFinalAlignHudSameLine.apply(this, args);
        finalAlignHudPerfect();
        return result;
      };
    }

    const oldUpdateMissionProgressCompact = typeof updateMissionProgress === 'function' ? updateMissionProgress : null;
    if (oldUpdateMissionProgressCompact) {
      updateMissionProgress = function(...args) {
        const result = oldUpdateMissionProgressCompact.apply(this, args);
        finalAlignHudPerfect();
        return result;
      };
    }
    ...
    finalAlignHudPerfect();
    setTimeout(finalAlignHudPerfect, 100);
    setTimeout(finalAlignHudPerfect, 300);


    function finalHudSafeAdjustment20260622() {
      const gameEl = document.getElementById('game');
      const width = gameEl && gameEl.getBoundingClientRect ? gameEl.getBoundingClientRect().width : window.innerWidth;
      const smallHud = width <= 720;
      const scoreCoinTopValue = smallHud ? '42px' : '48px';
      ...
      if (scoreEl) {
        scoreEl.style.setProperty('top', scoreCoinTopValue, 'important');
        scoreEl.style.setProperty('transform', 'translate(-50%, -50%)', 'important');
      }

      coinElements.forEach(el => {
        el.style.setProperty('top', scoreCoinTopValue, 'important');
        el.style.setProperty('transform', 'translateY(-50%)', 'important');
      });
    }

    const previousUpdateMissionProgress20260622 = typeof updateMissionProgress === 'function' ? updateMissionProgress : null;
    if (previousUpdateMissionProgress20260622) {
      updateMissionProgress = function(...args) {
        const result = previousUpdateMissionProgress20260622.apply(this, args);
        finalHudSafeAdjustment20260622();
        return result;
      };
    }

    const previousStartRound20260622 = typeof startRound === 'function' ? startRound : null;
    if (previousStartRound20260622) {
      startRound = function(...args) {
        const result = previousStartRound20260622.apply(this, args);
        finalHudSafeAdjustment20260622();
        setTimeout(finalHudSafeAdjustment20260622, 100);
        setTimeout(finalHudSafeAdjustment20260622, 300);
        return result;
      };
    }
    ...
    function forceDepthHiddenOutsideGame() {
      const shouldShowDepth = document.body.classList.contains('screen-playing');
      ...
      depthEls.forEach(el => {
        el.style.setProperty('display', shouldShowDepth ? '' : 'none', 'important');
        el.style.setProperty('visibility', shouldShowDepth ? 'visible' : 'hidden', 'important');
        el.style.setProperty('opacity', shouldShowDepth ? '1' : '0', 'important');
        el.classList.toggle('depth-ui-hidden', !shouldShowDepth);
      });
    }

    const oldStartRoundDepthFix = typeof startRound === 'function' ? startRound : null;
    if (oldStartRoundDepthFix) {
      startRound = function(...args) {
        const result = oldStartRoundDepthFix.apply(this, args);
        document.body.classList.remove('screen-start', 'screen-result', 'screen-prelevel', 'screen-shop');
        document.body.classList.add('screen-playing');
        forceDepthHiddenOutsideGame();
        return result;
      };
    }

    const oldShowPreLevelDepthFix = typeof showPreLevelScreen === 'function' ? showPreLevelScreen : null;
    if (oldShowPreLevelDepthFix) {
      showPreLevelScreen = function(...args) {
        const result = oldShowPreLevelDepthFix.apply(this, args);
        document.body.classList.remove('screen-playing', 'screen-start', 'screen-result', 'screen-shop');
        document.body.classList.add('screen-prelevel');
        forceDepthHiddenOutsideGame();
        return result;
      };
    }

    const oldFinishRoundDepthFix = typeof finishRound === 'function' ? finishRound : null;
    if (oldFinishRoundDepthFix) {
      finishRound = async function(...args) {
        document.body.classList.remove('screen-playing', 'screen-start', 'screen-prelevel', 'screen-shop');
        document.body.classList.add('screen-result');
        forceDepthHiddenOutsideGame();
        return await oldFinishRoundDepthFix.apply(this, args);
      };
    }

    document.body.classList.remove('screen-playing', 'screen-result', 'screen-prelevel', 'screen-shop');
    document.body.classList.add('screen-start');
    forceDepthHiddenOutsideGame();
    setTimeout(forceDepthHiddenOutsideGame, 100);
    setTimeout(forceDepthHiddenOutsideGame, 500);


    function forceHideDepthEverywhereExceptGameplay() {
      const isGameplay = document.body.classList.contains('screen-playing');

      const candidates = [
        document.getElementById('depthMeter'),
        document.getElementById('depthNumber'),
        ...Array.from(document.querySelectorAll('[id*="depth" i], [class*="depth" i], [id*="profund" i], [class*="profund" i], [aria-label*="profund" i]'))
      ].filter(Boolean);

      // Fallback visual: pega barras verticais altas do lado direito, como a barra que aparece na imagem.
      Array.from(document.querySelectorAll('div, span, aside')).forEach(el => {
        try {
          const rect = el.getBoundingClientRect();
          const style = window.getComputedStyle ? window.getComputedStyle(el) : null;
          const isRightVerticalBar =
            rect.width > 6 &&
            rect.width < 42 &&
            rect.height > 180 &&
            rect.left > window.innerWidth * 0.82 &&
            (style ? (style.position === 'absolute' || style.position === 'fixed') : true);

          if (isRightVerticalBar) candidates.push(el);
        } catch (error) {}
      });
      ...
      [...new Set(candidates)].forEach(el => {
        ...
        if (isGameplay) {
          el.classList.remove('depth-force-hidden');
          el.style.removeProperty('display');
          el.style.removeProperty('visibility');
          el.style.removeProperty('opacity');
          el.style.removeProperty('pointer-events');
        } else {
          el.classList.add('depth-force-hidden');
          el.style.setProperty('display', 'none', 'important');
          el.style.setProperty('visibility', 'hidden', 'important');
          el.style.setProperty('opacity', '0', 'important');
    ...
    function setScreenAndDepthVisibility(screenName) {
      document.body.classList.remove('screen-start', 'screen-prelevel', 'screen-playing', 'screen-result', 'screen-shop');
      document.body.classList.add(`screen-${screenName}`);
      forceHideDepthEverywhereExceptGameplay();
    }

    const depthFixOriginalStartRound = typeof startRound === 'function' ? startRound : null;
    if (depthFixOriginalStartRound) {
      startRound = function(...args) {
        const result = depthFixOriginalStartRound.apply(this, args);
        setScreenAndDepthVisibility('playing');
        setTimeout(forceHideDepthEverywhereExceptGameplay, 60);
        return result;
      };
    }

    const depthFixOriginalFinishRound = typeof finishRound === 'function' ? finishRound : null;
    if (depthFixOriginalFinishRound) {
      finishRound = async function(...args) {
        setScreenAndDepthVisibility('result');
        forceHideDepthEverywhereExceptGameplay();
        return await depthFixOriginalFinishRound.apply(this, args);
      };
    }

    const depthFixOriginalShowPreLevel = typeof showPreLevelScreen === 'function' ? showPreLevelScreen : null;
    if (depthFixOriginalShowPreLevel) {
      showPreLevelScreen = function(...args) {
        const result = depthFixOriginalShowPreLevel.apply(this, args);
        setScreenAndDepthVisibility('prelevel');
        return result;
      };
    }

    const depthFixOriginalOpenShop = typeof openShop === 'function' ? openShop : null;
    if (depthFixOriginalOpenShop) {
      openShop = function(...args) {
        const result = depthFixOriginalOpenShop.apply(this, args);
        setScreenAndDepthVisibility('shop');
        return result;
      };
    }

    const depthFixOriginalCloseShop = typeof closeShop === 'function' ? closeShop : null;
    if (depthFixOriginalCloseShop) {
      closeShop = function(...args) {
        const result = depthFixOriginalCloseShop.apply(this, args);
        const startVisible = startOverlay && !startOverlay.classList.contains('hidden');
        setScreenAndDepthVisibility(startVisible ? 'start' : 'playing');
        return result;
      };
    }

    setScreenAndDepthVisibility('start');
    forceHideDepthEverywhereExceptGameplay();
    setTimeout(forceHideDepthEverywhereExceptGameplay, 50);
    setTimeout(forceHideDepthEverywhereExceptGameplay, 250);
    setTimeout(forceHideDepthEverywhereExceptGameplay, 1000);
...
/* SINCRONIZAR HUD DE MISSÃO NO PERFIL */
    function ensureMissionProfileHudImage() {
      try {
        ...
        const img = document.createElement('img');
        img.src = './assets/perfil-missao.png';
        img.alt = '';
    ...
    function ensureCorrectMissionHudFinal() {
      try {
        ...
        if (!img) {
          ...
          img.src = './assets/hud-missao-perfil.png';
          img.alt = '';
    ...
    function syncFinalMissionHudLayout() {
      try {
        const mission = document.getElementById('missionProgress');
        if (mission) {
    ...
    /* PATCH DEFINITIVO TESTADO: aplica o HUD grande antes/depois de qualquer rotina antiga */
    (function(){
      const W_DESKTOP='clamp(560px, 56vw, 650px)'; const W_MOBILE='82vw';
      function compactMissionText(text){return String(text||'').replace(/^Pegue\s+/i,'').replace(/^Cavalos-marinhos/i,'Cavalos-marinhos').replace(/^Peixes dourados/i,'Peixes dourados').replace(/^Peixes/i,'Peixes');}
      function applyMissionHudStable(){try{
        const mission=document.getElementById('missionProgress'); const label=document.getElementById('missionProgressLabel'); const fill=document.getElementById('missionProgressFill'); const phase=document.getElementById('phaseNumber'); const mobile=window.innerWidth<=720;
        if(mission){mission.style.setProperty('position','absolute','important');mission.style.setProperty('left',mobile?'4px':'8px','important');mission.style.setProperty('top',mobile?'4px':'6px','important');mission.style.setProperty('right','auto','important');mission.style.setProperty('bottom','auto','important');mission.style.setProperty('width',mobile?W_MOBILE:W_DESKTOP,'important');mission.style.setProperty('min-width',mobile?'0':'560px','important');mission.style.setProperty('max-width','none','important');mission.style.setProperty('height','auto','important');mission.style.setProperty('aspect-ratio','1307 / 548','important');mission.style.setProperty('transform','none','important');mission.style.setProperty('background-image',"url('./assets/hud-missao-perfil-final.png')",'important');mission.style.setProperty('background-size','contain','important');mission.style.setProperty('background-repeat','no-repeat','important');mission.style.setProperty('background-position','left top','important');mission.style.setProperty('overflow','visible','important');mission.style.setProperty('border','0','important');mission.style.setProperty('box-shadow','none','important');mission.style.setProperty('z-index','120','important');if(document.body.classList.contains('screen-playing')){mission.style.setProperty('display','block','important');mission.style.setProperty('visibility','visible','important');mission.style.setProperty('opacity','1','important');}}
        if(label){label.style.setProperty('left','35.8%','important');label.style.setProperty('top','29.5%','important');label.style.setProperty('width','61%','important');label.style.setProperty('height','29%','important');label.style.setProperty('display','flex','important');label.style.setProperty('align-items','center','important');label.style.setProperty('justify-content','center','important');label.style.setProperty('font-size',mobile?'9px':'clamp(10px, 1.05vw, 15px)','important');label.style.setProperty('line-height','1','important');label.style.setProperty('white-space','nowrap','important');label.style.setProperty('overflow','hidden','important');label.style.setProperty('text-overflow','clip','important');if(label.textContent) label.textContent=compactMissionText(label.textContent);}
        if(fill){fill.style.setProperty('left','36.5%','important');fill.style.setProperty('top','35.9%','important');fill.style.setProperty('height','18%','important');fill.style.setProperty('max-width','58.5%','important');}
        if(phase){if(typeof currentPhase!=='undefined') phase.textContent=String(currentPhase);phase.style.setProperty('left',mobile?'5.4vw':'39px','important');phase.style.setProperty('top',mobile?'20.6vw':'148px','important');phase.style.setProperty('width','48px','important');phase.style.setProperty('height','38px','important');}
      }catch(e){}}
      window.__applyMissionHudStable=applyMissionHudStable;
      window.forceHudLayout=function(){applyMissionHudStable();try{const score=document.getElementById('liveScore'); if(score){score.style.left='50%';score.style.right='auto';score.style.top='38px';score.style.transform='translateX(-50%)';score.style.textAlign='center';}}catch(e){}};
      window.finalHudForceLayout=applyMissionHudStable; window.finalAlignHudSameLine=applyMissionHudStable; window.syncCleanMissionHud=applyMissionHudStable; window.syncFinalMissionHudLayout=applyMissionHudStable;
      const oldStart=typeof startRound==='function'?startRound:null; if(oldStart&&!startRound.__stableHudWrapped){startRound=function(...args){applyMissionHudStable(); const r=oldStart.apply(this,args); applyMissionHudStable(); requestAnimationFrame(applyMissionHudStable); setTimeout(applyMissionHudStable,0); setTimeout(applyMissionHudStable,60); setTimeout(applyMissionHudStable,160); return r;}; startRound.__stableHudWrapped=true;}
      const oldUpdate=typeof updateMissionProgress==='function'?updateMissionProgress:null; if(oldUpdate&&!updateMissionProgress.__stableHudWrapped){updateMissionProgress=function(...args){const r=oldUpdate.apply(this,args); applyMissionHudStable(); return r;}; updateMissionProgress.__stableHudWrapped=true;}
      const oldScreen=typeof setGameScreen==='function'?setGameScreen:null; if(oldScreen&&!setGameScreen.__stableHudWrapped){setGameScreen=function(...args){const r=oldScreen.apply(this,args); applyMissionHudStable(); return r;}; setGameScreen.__stableHudWrapped=true;}
      document.addEventListener('DOMContentLoaded',()=>{applyMissionHudStable(); requestAnimationFrame(applyMissionHudStable);}); applyMissionHudStable();
    })();


    /* PATCH EXTRA TESTADO: substitui os bindings globais antigos, não apenas window.* */
    (function(){
      function stable(){ if (typeof window.__applyMissionHudStable === 'function') window.__applyMissionHudStable(); }
      try { finalHudForceLayout = stable; } catch(e) {}
      try { finalAlignHudSameLine = stable; } catch(e) {}
      try { syncCleanMissionHud = stable; } catch(e) {}
      try { syncFinalMissionHudLayout = stable; } catch(e) {}
