    ...
    function normalizeCapturedKind(item) {
      ...
      if (raw.includes('seahorse') || raw.includes('cavalo') || raw.includes('turtle') || raw.includes('tartaruga') || raw.includes('shark') || raw.includes('tubar')) return 'normal';
    ...
    function registerMissionCapture(itemOrKind) {
      ...
      if (kind === 'giant') {
        triggerGiantImpactFeedback();
      }

      if (kind === 'gold') {
        missionGoldCaught += 1;
      } else if (kind === 'giant' || kind === 'bottle' || kind === 'chest' || kind === 'special') {
    ...
    function forceHudLayout() {
      ...
      if (mission) {
        mission.style.left = '16px';
        mission.style.right = 'auto';
        mission.style.top = '44px';
        mission.style.width = 'clamp(220px, 34vw, 340px)';
    ...
    function updateMissionProgress() {
      ...
      progressLabel.textContent = `${progress.label}: ${progress.current}/${progress.target}`;
    }
    ...
    function maybeSpawnSpecialItems() {
      ...
      if (specialSpawnState.giant < 1 && specialSpawnState.giantEnabled && progress > 0.55) {
        candidates.push({ type: 'giant', value: 1000, width: 190, height: 116, chance: 0.018 });
      }
    ...
    function getPreLevelFishGoalForPhase(phase) {
      ...
      if (safePhase % 10 === 0) {
        return {
          type: 'special',
          target: Math.min(3, 1 + Math.floor(safePhase / 40)),
          text: `Pegue ${Math.min(3, 1 + Math.floor(safePhase / 40))} especiais`
        };
      }
    ...
    function getMissionFishIcon() {
      ...
      if (fishGoal.type === 'normal') return './assets/fish-normal.png';
      return './assets/fish-gold.png';
    }
...
applyHookVisuals();
    ...
    function playRareAnimalScoreIfNeeded(fish) {
      if (!fish || !isRareAnimalType || !isRareAnimalType(fish.type)) return;
      showRareAnimalScoreFeedback(fish.type, fish.x, fish.y);
      try{
        if(typeof showScorePopup==='function'){ showScorePopup(fish.points||0, fish.x, fish.y); }
    ...
    const rareFishConfig = {
      seahorse: { points: 250, width: 36, speed: 0.85, label: 'Cavalo-marinho' },
    ...
    startBtn.addEventListener('click', showPreLevelScreen);
    ...
    function finalCompactMissionLabel(text) {
      ...
      else if (lower.includes('especial')) label = 'Itens especiais';
